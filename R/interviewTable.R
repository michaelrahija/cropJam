##' Takes the interview actions file, and tabulates by interview
##' 
##' 
##' @param x data.frame imported from interview actions file
##' @param format.time if TRUE, returns time in format D:H:M:S, otherwise secs
##' 
##'     
##' @return returns table with each row as unique interview, and columns containing various attributes
##'
##' @export
##' 

interviewTable <- function(x = para,
                           format.time = TRUE){
...
#     }
    ...
    if(!("Supervisor" %in% colnames(role.df))){
      role.df <- cbind(role.df, Supervisor = rep("Error: No Super action ", 
                                                 n = nrow(role.df)))
